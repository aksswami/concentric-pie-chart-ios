...
class ASPieChart: UIView {
    ...
    var descriptionTextFont: UIFont = UIFont.systemFontOfSize(14)
    
    var descriptionTextSelectedFont: UIFont = UIFont.boldSystemFontOfSize(14)
    
    var duration: NSTimeInterval = 2.0
    
    var labelDistanceFromCenter: CGFloat = 150
    ...
    private var sectorHighlightLabelLine: CAShapeLayer!
    
    private var selectedItem: ASPieChartDataItem!
    ...
    func createLabelLinesUsingOuterCircle() {
        ...
        for (_, circle) in circles.reverse().enumerate() {
            for item in circle.dataItems as [ASPieChartDataItem] {
                ...
                let labelLineFarEndPoint = getLabelLineFarEndPoint(forItem: item)
                ...
                path.addLineToPoint(labelLineFarEndPoint)
                let dashes: [CGFloat] = [ path.lineWidth * 0, path.lineWidth * 2 ]
                path.setLineDash(dashes, count: dashes.count, phase: 0)
                path.lineCapStyle = .Round
                ...
                if let descriptionLabel = descriptionLabelForItem(item) {
    ...
    func getLabelLineFarEndPoint(forItem item: ASPieChartDataItem) -> CGPoint{
        let labelStartPointRight = CGPointMake(circleCenter.x + labelDistanceFromCenter , item.labelCirclePoint.y)
        let labelStartPointLeft = CGPointMake(circleCenter.x - labelDistanceFromCenter , item.labelCirclePoint.y)
        return (Double(item.itemCenterAngel) > -M_PI_2  &&  Double(item.itemCenterAngel) < M_PI_2 ) ? labelStartPointRight : labelStartPointLeft
    }
    
    func descriptionLabelForItem(item: ASPieChartDataItem) -> UILabel? {
        let labelFarEndPoint = getLabelLineFarEndPoint(forItem: item)
        let labelWidth: CGFloat = 60.0
        let labelHeight: CGFloat = 50.0
        if let textDescription = item.textDescription {
            let xCord: CGFloat = (Double(item.itemCenterAngel) > -M_PI_2  &&  Double(item.itemCenterAngel) < M_PI_2 ) ? labelFarEndPoint.x : labelFarEndPoint.x - labelWidth
        
            let label: UILabel = UILabel.init(frame: CGRectMake(xCord, item.labelCirclePoint.y - (labelHeight * 0.5), labelWidth, labelHeight))
            let formatPrecision = ".2"
            label.text = textDescription + "\n \(item.itemPercentage.format(formatPrecision))%"
    ...
    func didTouch(atLocation touchLocation: CGPoint) {
        ...
        if shouldHighlightSectorOnTouch {
            ...
            sectorHighlight = newCircleLayerWithRadius(currentCircle.outerRadius + 5, borderWidth: 10, fillColor: UIColor.clearColor(), borderColor: newColor, startPercentage: startPercentage, endPercentage: endPercentage)
            selectedItem = currentItem
            selectedItem.descriptionLabel.font = descriptionTextSelectedFont
            
            sectorHighlightLabelLine = CAShapeLayer.init()
            
            let path = UIBezierPath()
            path.moveToPoint(currentItem.itemCenterPoint)
            path.addLineToPoint(currentItem.labelCirclePoint)
            path.addLineToPoint(getLabelLineFarEndPoint(forItem: currentItem))
            sectorHighlightLabelLine.fillColor = UIColor.clearColor().CGColor
            sectorHighlightLabelLine.strokeColor = currentItem.color.CGColor
            sectorHighlightLabelLine.lineWidth = 3
            sectorHighlightLabelLine.path = path.CGPath
            ...
            if self.enableMultipleSelection {
                ...
                contentView.layer.addSublayer(sectorHighlightLabelLine)
            }
        }
    }
    ...
    func unselectItem() {
        ...
        selectedItem?.descriptionLabel?.font = descriptionTextSelectedFont
        sectorHighlightLabelLine?.removeFromSuperlayer()
